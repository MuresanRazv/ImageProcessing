...
Point::Point(unsigned int x_coord, unsigned int y_coord) {
	x = x_coord;
	y = y_coord;
}
...
Image Image::getROI(Image& roiImg, unsigned int x, unsigned int y, unsigned int width, unsigned int height) {
	...
	// Check if it crosses to boundary
	if (x + roiImg.width() - m_width < 0) {
		roiImg = Image();
	}
	else if (y + roiImg.height() - m_height < 0) {
		roiImg = Image();
	}

	else {
		// Build the new result image
		for (int h = result_point.y, r_h = 0; h < result_point.y + roiImg.height(); h++, r_h++)
			for (int w = result_point.x, r_w = 0; w < result_point.x + roiImg.width(); w++, r_w++) {
				roiImg.m_data[r_h][r_w] = m_data[h][w];
			}
	}
